...
- **今日最优惠活动**：原「电商购物」五个 icon 入口下线，改为标题「今日最优惠活动」+ 一行三个 banner 坑位，每个坑位为「图标（image） + 促销文案」两段式结构；数据由接口下发（接口正在开发中，期望响应 `[{ id, image, title, link?, targetType? }]`，最多取前 3 个），对接完成后在 `attached` 启用 `loadActivityBanners()`；接口未就绪前展示占位文案「活动即将上线，敬请期待」
