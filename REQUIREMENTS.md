...
- **登录流程**：采用按需拦截模式（`ensureLogin`），打开小程序不自动弹登录窗。用户执行搜索、转链、点击平台入口或商品时触发登录检测，弹窗文案「欢迎使用值物APP，为了能体验完整的服务，请您进行登录」。登录成功后自动重试之前触发的操作
...
| UI 渐变 | `#ff4d00 → #ff9402` | 头部背景、tab、tab 选中胶囊、主按钮渐变 |
| 主文字 | `#333333` | 标题、主要信息 |
